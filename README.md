...
| `brainstorm-cdc` | Explore une idée de produit encore floue jusqu'à un cahier des charges qui fait foi — divergence en conversation ouverte (où l'agent contredit et propose un angle non demandé), verrouillage de chaque décision par question fermée, puis rédaction d'un `cdc.md` dont chaque décision porte sa justification et chaque inconnue son critère de levée. En amont d'`init-projet`. |
| `maquette-vers-produit` | Porte une maquette (doc design, Figma, HTML exporté) dans un produit qui **existe déjà** — inventaire exhaustif écran par écran en cases à cocher, vérification du câblage backend à trois niveaux (route / appel / charge), backlog de specs, livraison déléguée au trio TDD, revue visuelle sur la plateforme cible. |
